...
#define JOY_STICK_X      3
#define JOY_STICK_Y      4
#define TxD              6
#define RxD              7
#define LED_PIN          8

// Connection status on the SeeedStudio v1 shield is available on pin A1
...
void setup() {
  ...
  else
  {
    ...
    // Set up the local (master) Bluetooth module
    setupBlueToothConnection();
    // Search for (MAC) address of slave
    getSlaveAddress();
    // Execute the connection to the slave
...
void loop()
{
  ...
  while (blueToothSerial.available())
  {
    ...
    else if (recvCommand.length() > 0 && recvChar == ')')
    {
      recvCommand += ')';
      if (recvCommand.equals("(PING,PONG)"))
      {
        digitalWrite(LED_PIN, HIGH);
      }
      recvCommand = "";
    }
    else if (recvCommand.length() > 0)
    {
      recvCommand += recvChar;
    }
  }
...
String readJoyStick()
{
  int x = analogRead(JOY_STICK_X);
  int y = 1023 - analogRead(JOY_STICK_Y);
  int z = 0;
...
String readIRControl()
{
  ...
  if (irrecv.decode(&results))
  {
    ...
    if (results.value == BUTON_2)
    {
      cmd = "(AUTO,,)";
    }

    if (results.value == BUTON_5)
    {
      cmd = "(MEM_FWD,,)";
    }

    if (results.value == BUTON_7)
    {
      digitalWrite(LED_PIN, LOW);
    }

    if (results.value == BUTON_8)
    {
      cmd = "(MEM_BAK,,)";
    }

    if (results.value == BUTON_9)
    {
      int x = analogRead(JOY_STICK_X);
      int y = 1023 - analogRead(JOY_STICK_Y);
      cmd = "(ADJ_XY," + String(x) + "," + String(y) + ")";
    }
